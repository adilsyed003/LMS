...
export async function POST(req: Request) {
  const { text, count: rawcount } = await req.json();
  ...
  const prompt = `
You are a teacher. Read the following text and create a multiple choice test with ${count} questions based on its content.
Each question should be an object with:
- "question": string,
- "options": array of 4 strings,
- "answer": "A", "B", "C", or "D" (the correct option).
Return a JSON array of ${count} such objects, and nothing else.
